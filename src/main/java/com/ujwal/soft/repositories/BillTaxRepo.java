...
public interface BillTaxRepo extends JpaRepository<TaxBillBean, Integer> {
	...
	@Query(value = "SELECT bill.bill_detail_id, billhead.bill_header_id, billhead.company_id, billhead.cust_id, billhead.bill_date, cust.cust_name, sum(bill.taxable_amount) as taxable_amt, (bill.cgst_per + bill.sgst_per) as tax_per,\r\n" + 
			...
			"			\r\n" + 
			"			WHERE billhead.company_id =:comp_id AND bill.bill_header_id=billhead.bill_header_id AND billhead.bill_date BETWEEN :fromDate AND :toDate AND  cust.cust_id = billhead.cust_id\r\n" + 
			"			\r\n" + 
			"			group by   billhead.bill_header_id,tax_per, bill.ex_var1",nativeQuery=true)
	...
	@Query(value = "SELECT bill.bill_detail_id, billhead.bill_header_id, billhead.company_id, billhead.cust_id, billhead.bill_date, cust.cust_name, sum(bill.taxable_amount) as taxable_amt, (bill.cgst_per + bill.sgst_per) as tax_per,\r\n" + 
			...
			"			FROM bill_details bill, bill_header billhead, m_customer cust\r\n" + 
			"			\r\n" + 
			"			WHERE bill.bill_header_id=billhead.bill_header_id AND billhead.bill_date BETWEEN :fromDate AND :toDate AND  cust.cust_id = billhead.cust_id\r\n" + 
			"			\r\n" + 
			"			group by   billhead.bill_header_id,tax_per, bill.ex_var1",nativeQuery=true)
