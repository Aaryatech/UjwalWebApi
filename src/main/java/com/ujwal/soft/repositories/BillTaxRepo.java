...
public interface BillTaxRepo extends JpaRepository<TaxBillBean, Integer> {
	...
	@Query(value = "SELECT bill.bill_detail_id, billhead.bill_header_id, billhead.company_id, billhead.cust_id, billhead.bill_date, cust.cust_name, bill.taxable_amount, "
			+ "cust.cust_gstn,bill.cgst_per, bill.sgst_per, bill.igst_per, sum(bill.taxable_amount) AS taxableAmount, bill.grand_total as billAmount \r\n" + 
	...
	@Query(value = "SELECT bill.bill_detail_id, billhead.bill_header_id, billhead.company_id, billhead.cust_id, billhead.bill_date, cust.cust_name, bill.taxable_amount,bill.grand_total as billAmount, "
			+ "cust.cust_gstn, bill.cgst_per, bill.sgst_per, bill.igst_per, sum(bill.taxable_amount) AS taxableAmount \r\n" + 
			"FROM bill_details bill, bill_header billhead, m_customer cust\r\n" + 
