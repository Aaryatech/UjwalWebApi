...
public interface ItemRepo extends JpaRepository<ItemBean, Integer>{
	@Query(value = "select d.bill_detail_id,d.bill_header_id, d.part_id, p.part_name,t.hsn_code,t.cgst_per,t.sgst_per,t.igst_per,\r\n" + 
			...
			"h.cust_id, c.cust_name, h.bill_date, h.tax_per, sum(d.qty) as qty,sum(d.cgst_rs) as cgst,sum(d.sgst_rs) as sgst, \r\n" + 
			...
			"WHERE d.bill_header_id=h.bill_header_id AND d.part_id=p.part_id AND h.bill_date BETWEEN :fromDate AND :toDate \r\n" + 
			
			"AND h.cust_id=c.cust_id AND t.tax_id=p.part_tax_id AND d.part_id = :itemId GROUP BY h.bill_date, d.part_id",nativeQuery=true)  
